...
public class StringUtil {
    ...
    public static Map.Entry<String, String> getAppropriateVerbAndSuffix(
            Collection<?> collection, Set<String> verbs) {
        if (collection.isEmpty())
            return new AbstractMap.SimpleImmutableEntry<>("", "");
        String appropriateVerb;
        if (verbs.contains("have") || verbs.contains("has"))
            appropriateVerb = collection.size() == 1 ? "has" : "have";
        else if (verbs.contains("is") || verbs.contains("are"))
            appropriateVerb = collection.size() == 1 ? "is" : "are";
        else
            throw new NotSupportedException(
                    "Target verb(s) not supported: " + verbs);
        String pluralOrSingularChar = collection.size() == 1 ? "" : "s";
        return new AbstractMap.SimpleImmutableEntry<>(
    ...
    public static String tryDetectEncoding2(byte[] encodedBytes) {
        ...
        charsetDetector.Init(detectedCharsetAtomic::set);
        charsetDetector.DoIt(encodedBytes, encodedBytes.length, false);
        charsetDetector.DataEnd();
        return detectedCharsetAtomic.get();
    }

    public static String tryDetectEncoding(byte[] encodedBytes) {
